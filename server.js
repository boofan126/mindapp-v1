...
// ---------- 定时任务：每天下午13点运行一次（北京时间 13:00 对应 UTC 5:00，但保留原设定）----------
...
app.post('/api/auto-checkin', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE tasks SET "lastCheckin" = $1, "warningSent" = 0, "finalSent" = 0, "warningTriggeredAt" = NULL WHERE user_email = $2`,
      [now, email]
    );
...
app.listen(port, '0.0.0.0', () => {
  ...
  pool.query('SELECT NOW()', (err, res) => {
    if (err) {
      ...
      console.log('✅ 成功连接到 PostgreSQL，服务器时间：', res.rows[0].now);
    }
  });
});